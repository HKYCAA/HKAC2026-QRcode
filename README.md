...
7. The matching row in `all` is highlighted green and its `Timestamp` cell
   records the student's arrival date and time.
8. A short script lock covers only duplicate checking and attendance writes,
   making simultaneous check-ins safe without serializing roster lookups.
9. **Undo check-in** asks for confirmation, removes the matching attendance
   row, and clears the roster timestamp and green highlight.
...
- `Timestamp` — arrival date and time written by the check-in backend.
